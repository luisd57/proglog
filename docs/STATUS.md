...
- **Symfony API (`API/`)**: all 6 domains ported (exercises, templates, sessions, measurements, profile, stats). Booted, migrated, seeded (873 exercises); 412/412 PHPUnit green (2026-08-05); endpoints smoke-tested against the contract (2026-08-04).
- **Web**: restructured to domain layout, adapted to the new contract. `ng build` clean, 53/53 Vitest passing. Dev proxy wired (`serve.options.proxyConfig`).
