...
@Component({
  ...
  template: `
    <header class="page-header">
      ...
      @if (data.value()?.bodyweight_kg; as bw) {
            ...
            <li class="card">
              <div class="row-head">
                <span class="name">
                  @if (entry.exercise_id) {
                    <a [routerLink]="['/exercises', entry.exercise_id]">{{ entry.label }}</a>
                  } @else {
                    {{ entry.label }}
                  }
                </span>
              ...
              @if (entry.e1rm !== null) {
                <p class="count">
                  ...
                  @if (entry.next_level) {
                    · {{ entry.progress! * 100 | number: '1.0-0' }}% to {{ entry.next_level }}
...
export class StrengthPage {
  ...
  nextThreshold(entry: { thresholds: number[]; next_level: string | null }): number {
    const levels = ['beginner', 'novice', 'intermediate', 'advanced', 'elite'];
    const index = levels.indexOf(entry.next_level ?? '');
