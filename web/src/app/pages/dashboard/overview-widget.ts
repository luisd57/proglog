...
import { OverviewResult, StatsApi } from '../../services/stats-api';
...
export class OverviewWidget {
  ...
  readonly overview = rxResource({
    ...
    stream: ({ params }) => this.statsApi.overview(params.period),
  });
  ...
  readonly chartPoints = computed<ChartPoint[]>(() => {
    const o = this.view();
    if (!o) return [];
    return o.cumulativeVolume.map((p) => ({ label: p.date.slice(5), value: p.value }));
  });
  ...
  readonly cells = computed<StatCell[]>(() => {
    ...
    return [
      ...
      this.cell(o, 'Lifted', 'volumeKg', (n) => `${Math.round(n).toLocaleString()} kg`),
      this.cell(o, 'Reps', 'reps', (n) => `${n}`),
      this.cell(o, 'Sets', 'sets', (n) => `${n}`),
      this.cell(o, 'Heaviest', 'heaviestKg', (n) => `${n} kg`),
      this.cell(o, 'Time', 'timeSeconds', (n) => `${formatDuration(n)} h`),
    ];
  });
