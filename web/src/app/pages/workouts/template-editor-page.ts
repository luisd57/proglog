          ...
          <ul class="picker">
            @for (ex of found.slice(0, 12); track ex.id) {
              <li>
                <button (click)="add(ex)">
                  + {{ ex.name }}
                  <span class="chip">{{ ex.primaryMuscles.join(', ') }}</span>
                </button>
              </li>
            }
          </ul>
...
export class TemplateEditorPage {
  ...
  readonly highlights = computed(() => {
    const primary = this.rows().flatMap((r) => r.exercise.primaryMuscles);
    const secondary = this.rows().flatMap((r) => r.exercise.secondaryMuscles);
      ...
      if (template && !this.populated) {
        ...
        this.rows.set(
          template.exercises.map((te) => ({
            ...
            restSeconds: te.restSeconds ?? undefined,
          })),
        );
      }
    });
