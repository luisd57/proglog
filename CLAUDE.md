...
All 8 phases complete (2026-06-12). Suites: 60 API unit + 6 API e2e (Jest), 49 web (Vitest).

Exercise search (`ExercisesService.list`) is tokenized: the query is split into words, each lightly de-pluralized, and every token must appear as a substring of the name (ANDed, any order). This surfaces "Chin-Up" for `chin ups`, "Front Cable Raise" for `front raise`, etc. Tradeoff: pure substring means short tokens can match mid-word (`chin` ⊂ `machine`); acceptable noise for single-user, results stay alphabetical.
